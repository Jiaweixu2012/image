...
bmp_create_from_file(const char *filename){
	...
	result = malloc(sizeof(bmpfile_t));
	fread(&(result->header), sizeof(bmp_header_t), 1, file);
	fread(&(result->dib), sizeof(bmp_dib_v3_header_t), 1, file);

	result->colors = malloc(sizeof(rgb_pixel_t *) * result->dib.ncolors);
	fread(result->colors, sizeof(*result->colors), 1, file);

	bmp_malloc_pixels(result);
	fread(result->pixels, sizeof()*result->dib.bmp_bytesz, 1, file);
